...
public partial class Admin
{
    ...
    [
        ...
        new("start_date",  "Stipulated Start",       "Stip.Start", DefaultOn: false),
        new("exp_comp",    "Stipulated Completion",  "Stip.Comp",  DefaultOn: false),
        new("act_start",   "Actual Start",           "Act.Start",  DefaultOn: false),
        new("act_comp",    "Actual Completion",      "Act.Comp",   DefaultOn: false),
        new("fin_year",    "Financial Year",         "FY",         DefaultOn: false),
        new("progress",    "Physical Progress (%)",  "Phys%",      DefaultOn: true),
        new("fin_progress","Financial Progress (%)","Fin%",        DefaultOn: false),
        new("sanc_amt",    "A&F Amount (Lakhs)",     "A&F(L)",     DefaultOn: true),
        new("anf_no",      "A&F No.",                "A&F No",     DefaultOn: false),
        new("anf_date",    "A&F Date",               "A&F Dt",     DefaultOn: false),
        new("agr_amt",     "Work Order Amt (Lakhs)", "WO(L)",      DefaultOn: false),
        new("wo_no",       "Work Order No.",         "WO No",      DefaultOn: false),
        new("wo_date",     "Work Order Date",        "WO Date",    DefaultOn: false),
    ...
    private string ColValue(Work w, string key, int serial) => key switch
    {
        ...
        "act_start"    => w.ActualStartDate?.ToString("dd/MM/yyyy") ?? "",
        "act_comp"     => w.ActualCompletion?.ToString("dd/MM/yyyy") ?? "",
        "fin_year"     => w.FinancialYear ?? "",
        ...
        "anf_no"       => w.AnfNo      ?? "",
        "anf_date"     => w.AnfDate?.ToString("dd/MM/yyyy") ?? "",
        "agr_amt"      => w.AgreementAmount.HasValue  ? $"{w.AgreementAmount:F2}"  : "",
        "wo_no"        => w.WorkOrderNo  ?? "",
        "wo_date"      => w.WorkOrderDate?.ToString("dd/MM/yyyy") ?? "",
